...
// ────── Task 2 ──────
  ...
  const handleCopy = useCallback((event) => {
    // Don't copy if we are typing inside a cell or nothing is selected
    if (!selectedCell || editingCell) return;

    const cellData = engine.getCell(selectedCell.r, selectedCell.c);
    // Requirement: Copy computed results, not the raw formula
    const valueToCopy = cellData.error ? cellData.error : (cellData.computed !== null ? String(cellData.computed) : cellData.raw);
  ...
  const handlePaste = useCallback((event) => {
    ...
    // Split by newlines for rows, then by tabs for columns (Excel/Sheets format)
    const rows = text.split(/\r?\n/).filter(row => row !== "");
    
    rows.forEach((rowText, rowOffset) => {
      const columns = rowText.split('\t');
      columns.forEach((value, colOffset) => {
        const targetRow = selectedCell.r + rowOffset;
        const targetCol = selectedCell.c + colOffset;

        if (targetRow < engine.rows && targetCol < engine.cols) {
          engine.setCell(targetRow, targetCol, value);
        }
      });
    });
...
// Listen for global Copy/Paste events
  useEffect(() => {
  ...
  const visibleRows = useMemo(() => {
    ...
    // Filter Logic
    Object.entries(filters).forEach(([colIdx, query]) => {
      if (!query) return;
      const q = query.toLowerCase();
      indices = indices.filter(rowIndex => {
        const cell = engine.getCell(rowIndex, parseInt(colIdx));
  ...
  const handleSort = (colIndex) => {
    setSortConfig(prev => {
      if (prev.col !== colIndex) return { col: colIndex, direction: 'asc' };
                  ...
                  {Array.from({ length: engine.cols }, (_, colIndex) => (
                    <th key={colIndex} className="col-header">
                      <div className="header-container">
                        <div className="header-label" onClick={() => handleSort(colIndex)}>
                          {getColumnLabel(colIndex)}
                          <span className="sort-icon">
                            {sortConfig.col === colIndex ? (sortConfig.direction === 'asc' ? ' none' : ' desc') : ' asc'}
                          </span>
                        </div>
                        <input 
